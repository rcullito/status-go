...
func TestChildForPurpose(t *testing.T) {
	...
	bip44Child, err := masterKey.EthBIP44Child(index)
	if err != nil {
		t.Error("Error dering BIP44-compliant key")
	}

	walletChild, err := masterKey.ChildForPurpose(KeyPurposeWallet, index)
	if err != nil {
		t.Error("Error dering BIP44-compliant key")
	}

	chatChild, err := masterKey.ChildForPurpose(KeyPurposeChat, index)
	if err != nil {
